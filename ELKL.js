...
(function() {
  window.ELKL = window.ELKL || {};

  try {
    ELKL.browser = browser;
  } catch {
    ELKL.browser = chrome;
  }

  if (ELKL.browser.storage.sync) {
    ELKL.storage = ELKL.browser.storage.sync;
  } else {
    ELKL.storage = ELKL.browser.storage.local;
  }

  ELKL.Rule = class Rule {
    constructor(value) {
      this.value = value || '';
    }
  ...
  ELKL.Rules = class Rules extends Set {
    static get [Symbol.species]() { return Set; }

    static deserialize(values) {
      const rules = values.map((v) => new ELKL.Rule(v));
